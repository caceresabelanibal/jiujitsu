...
$tasks = [
    'emails'       => ['📨', 'Procesa la cola de mails (verificaciones, certificados)', '* * * * *'],
    'certificates' => ['📜', 'Genera los PDFs de certificados pendientes por lotes', '*/5 * * * *'],
    'rankings'     => ['📊', 'Recalcula el ranking global de competidores', '0 * * * *'],
    'cleanup'      => ['🧹', 'Borra inscripciones no verificadas (>72h) y mails viejos', '0 4 * * *'],
];
$key = CRON_KEY;
...
<div class="card">
  ...
  <div class="table-wrap"><table>
    ...
    <tr>
      ...
      <td><a class="btn sm" href="<?= APP_URL ?>/cron.php?task=<?= $name ?>&key=<?= e($key) ?>" target="_blank">▶ <?= t('run_now') ?></a></td>
    </tr>
    <?php endforeach; ?>
  </table></div>
</div>
...
<div class="card">
  <h3>📨 <?= t('emails_pending') ?>: <?= (int)scalar('SELECT COUNT(*) FROM email_queue WHERE status = "pending"') ?></h3>
  <div class="table-wrap"><table>
